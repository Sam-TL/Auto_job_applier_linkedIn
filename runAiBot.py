...
from selenium.common.exceptions import NoSuchElementException, ElementClickInterceptedException, NoSuchWindowException, ElementNotInteractableException, WebDriverException, TimeoutException, StaleElementReferenceException
...
from modules.resume_parser import find_years_for_label
...
def answer_common_questions(label: str, answer: str) -> str:
    keyword = next((value for key, value in question_keyword_answers.items() if key in label), None)
...
def _apply_text_placeholders(text: str, job_title: str, company_name: str, current_date: str) -> str:
    """
    Replace simple placeholders in user-provided snippets.
    Supported tokens:
        {job_title}    -> LinkedIn job title currently being applied to
        {company_name} -> Company name for the job listing
        {current_date} -> Date string formatted as supplied by caller
    """
    if not text:
        return text
    return (
        text.replace("{job_title}", job_title or "")
            .replace("{company_name}", company_name or "")
            .replace("{current_date}", current_date or "")
    )
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            if overwrite_previous_answers or selected_option == "Select an option" or force_overwrite:
                ...
                def _select_option(target: str) -> bool:
                    target_clean = target.strip().lower()
                    for option in select.options:
                        value = (option.get_attribute("value") or "").strip().lower()
                        text_value = (option.text or "").strip().lower()
                        if target_clean and (value == target_clean or text_value == target_clean):
                            option.click()
                            return True
                    return False
                ...
                try:
                    ...
                    selected = _select_option(answer)
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                if keyword_answer is not None:
                    ...
                    resume_years = find_years_for_label(label_org)
                    answer = resume_years if resume_years else years_of_experience
                elif 'phone' in label or 'mobile' in label: answer = phone_number
                elif 'street' in label: answer = street
                elif 'city' in label or 'location' in label or 'address' in label:
                    answer = preferred_location or current_city or work_location
                    do_actions = True
        ...
        if text_area:
            ...
            if not prev_answer or overwrite_previous_answers:
                keyword_answer = next((value for key, value in textarea_keyword_answers.items() if key in label), None)
                if keyword_answer is not None:
                    answer = keyword_answer
                elif 'summary' in label: answer = _apply_text_placeholders(linkedin_summary, job_title, company_name, current_date)
                elif 'cover' in label: answer = _apply_text_placeholders(cover_letter, job_title, company_name, current_date)
                if answer == "":
                ##> ------ Yang Li : MARKYangL - Feature ------
                    if use_AI and aiClient:
                        try:
...
return processed;
"""
        processed = driver.execute_script(script, modal, follow_companies)
        if processed:
            return
...
def run(total_runs: int) -> int:
    ...
    if not dailyEasyApplyLimitReached:
        delay = randint(300, 480)  # already imported at top
        print_lg(f"Sleeping for {delay//60} min…")
        sleep(delay)
    buffer(3)
    return total_runs + 1
...
def main() -> None:
    try:
        ...
        msg = f"\n{quote}\n\n\nBest regards,\nSamuel Thomas\nhttps://www.linkedin.com/in/samuel-thomas/\n\n"
