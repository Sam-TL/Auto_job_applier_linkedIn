...
def load_resume_ingestion() -> Dict[str, Any]:
    """
    Return the resume ingestion configuration from the active profile.
    """
    profile = _load_profile()
    value = profile.get("resume_ingestion", {})
    if not isinstance(value, dict):
        raise ValueError("`resume_ingestion` section must be a JSON object.")
    return value


__all__ = ["load_section", "list_missing_fields", "load_resume_ingestion"]
