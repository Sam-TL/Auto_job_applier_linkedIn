...
def ai_answer_question(
    ...
    try:
        user_info = user_information_all.strip() if user_information_all else None
        prompt = ai_answer_prompt.format(user_info or "N/A", question)

        if options and question_type in ['single_select', 'multiple_select']:
            options_block = "OPTIONS:\n" + "\n".join(f"- {option}" for option in options)
            prompt += f"\n\n{options_block}"
            if question_type == 'single_select':
                prompt += "\n\nSelect exactly ONE option name from the list above and answer with that option only."
            else:
                prompt += "\n\nSelect the option names that apply from the list above. If more than one, separate them with commas."
        ...
        print_lg(
            f"Prepared AI prompt for question '{question}'. "
            f"context -> user_info:{bool(user_info)}, options:{len(options) if options else 0}, "
            f"job_description:{bool(job_description and job_description != 'Unknown')}, "
            f"about_company:{bool(about_company and about_company != 'Unknown')}"
        )
