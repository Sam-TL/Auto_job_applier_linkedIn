...
'''
from modules.profile_loader import list_missing_fields


def _ensure_profile_fields(section: str, fields: list[str]) -> None:
    missing = list_missing_fields(section, fields)
    if missing:
        joined = ", ".join(missing)
        raise ValueError(
            f"Active profile is missing required fields for '{section}': {joined}.\n"
            "Update your JSON profile in 'config/profiles/' (or the file referenced by "
            "JOB_APPLIER_PROFILE/JOB_APPLIER_PROFILE_FILE) before running the bot."
        )
...
def validate_personals() -> None | ValueError | TypeError:
    ...
    _ensure_profile_fields(
        "personals",
        ["first_name", "last_name", "phone_number", "current_city", "country"]
    )
    ...
    check_string(preferred_email, "preferred_email")
    check_string(preferred_location, "preferred_location")
...
def validate_questions() -> None | ValueError | TypeError:
    ...
    _ensure_profile_fields(
        "questions",
        ["default_resume_path", "years_of_experience", "require_visa", "linkedIn", "us_citizenship"]
    )
